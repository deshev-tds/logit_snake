...
if __name__ == '__main__':
    cache_path = os.path.join(cache_path, "v017_拆分_claims_评测3")
    os.makedirs(cache_path, exist_ok=True)

    samples = []

    data_path = os.path.join(data_path, "v017_拆分_claims_评测3.xlsx")
    df = pd.read_excel(data_path, sheet_name="Sheet1")
    for i, line in tqdm(df.iterrows(), total=len(df), desc="v017_拆分_claims_评测3"):
        reason = line['原因']
        reason = reason if isinstance(reason, str) else ""

        answer = line['评测3 （0131版本 KP1）']
    ...
    check_keys(samples)
    write(samples, "v017_拆分_claims_评测3.jsonl")
