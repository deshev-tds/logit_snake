...
if __name__ == '__main__':
    cache_path = os.path.join(cache_path, "v017_拆分_claims_评测1")
    os.makedirs(cache_path, exist_ok=True)

    samples = []

    data_path = os.path.join(data_path, "v017_拆分_claims_评测1.xlsx")
    df = pd.read_excel(data_path, sheet_name="Sheet1")
    for i, line in tqdm(df.iterrows(), total=len(df), desc="v017_拆分_claims_评测1"):
        reason = line['原因']
    ...
    check_keys(samples)
    write(samples, "v017_拆分_claims_评测1.jsonl")
