...
class NPM:
    def __init__(self, model_path, device_map):
        ...
        self.model = AutoModelForMaskedLM.from_pretrained(model_path, device_map=device_map)
        self.max_length = 512  # npm-single only support 512 tokens

        with open("./methods/retrieval/FActScore/roberta_stopwords.txt", "r", encoding='utf-8') as f:
            self.stopwords = set()
            for line in f:
    ...
    def tokenize(self, texts, skip_special_tokens=False, padding=True):
        ...
        all_input_ids = self.tokenizer(texts, max_length=self.max_length)["input_ids"]
    ...
    def encode(self, texts, skip_special_tokens=False, gt_input_ids=None):
        ...
        outputs = self.model(all_input_ids.to(self.model.device),
                             all_attention_mask.to(self.model.device),
    ...
    def get_probability(self, question, retrievals):
        encoded = self.encode(retrievals, skip_special_tokens=True)
        ...
        mini_bs = 32
        mini_batches, mini_batches_gt_input_ids = [], []
        for i in range(math.ceil(len(batch) / mini_bs)):
            mini_batches.append(batch[i * mini_bs: (i + 1) * mini_bs])
            mini_batches_gt_input_ids.append(gt_input_ids[i * mini_bs: (i + 1) * mini_bs])

        for mini_batch, mini_batch_gt_input_ids in zip(mini_batches, mini_batches_gt_input_ids):
            for (prob, vector), gt_input_id in zip(self.encode(mini_batch, gt_input_ids=mini_batch_gt_input_ids), mini_batch_gt_input_ids):
